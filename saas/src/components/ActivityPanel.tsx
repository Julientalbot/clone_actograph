      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {activities.map((activity) => (
          <button
            ...
            className={`${activity.color} p-5 rounded-2xl text-white font-medium transition-all duration-300 transform hover:scale-[1.02] shadow-lg hover:shadow-xl`}
          >
      ...
      <div className="border-t border-slate-200 dark:border-slate-700 pt-8">
        ...
        <div className="space-y-3 max-h-80 overflow-y-auto">
          {events.length === 0 ? (
            ...
            events.map((event) => (
              <div key={event.id} className="flex items-center justify-between p-4 bg-white/60 backdrop-blur-sm rounded-xl border border-white/20 hover:bg-white/80 transition-all duration-300">
